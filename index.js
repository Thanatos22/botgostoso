	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Coe @${num.split('@')[0]} sou o 𝜝𝛷𝑻 𝑻𝑯𝜟𝑵𝜟𝑻𝛷𝑺 e seja bem vindo ao grupo *${mdata.subject}* lembre-se de ler a desgraça das regras`
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: 'Este comando só pode ser usado pelo dono/criador do grupo ',
					ownerB: 'Este comando só pode ser usado pelo meu dono ',
					admin: 'Este comando só pode ser usado por administradores do grupo ',
					Badmin: 'Este comando só pode ser usado quando o bot for adm '
				}
			}
			...
			switch(command) {
				...
				case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('só funciona com foto macaco')
					}
					break
				case 'stiker':
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`Falha ao converter $ {type} em figurinha`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('Deu merda carai, tenta dnv daqui a pouco')
							})
					...
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Cade a porra do codigo do idioma macaco', text, {quoted: mek})
				...
				case 'tagall':
					...
					teks += '\n\n'
					for (let mem of groupMembers) {
						teks += `𖡶 https://wa.me/${mem.jid.split('@')[0]}\n`
						members_id.push(mem.jid)
					}
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Membro comum @${mentioned[0].split('@')[0]} promovido a adm com sucesso, to de olho filho da puta 🧐`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Adm @${mentioned[0].split('@')[0]} rebaixado a membro comum com sucesso 😳`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`B A N I D O @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista dos adms of *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Recurso de boas vindas ativado️')
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Marca um cara ai pra eu clonar')
